...
class SchedulesViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, CalendarDelgate, UITableViewDelegate, UITableViewDataSource , UISearchBarDelegate  {
    ...
    func startObservingDatabase () {
        ...
        databaseService.getEvents() { (data) in
            self.getUpdatedSchedules(){ (data) in
                if var eventData = data {
                    eventData = self.updateExpired(events: eventData)
                    
                    self.oldevents = eventData
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let cellData = self.calendarArray?[indexPath.row] as! [String: Any]
        monthShownLabel.text = "\(String(describing: cellData["monthValue"] as! String)) \(cellData["year"] as! String)"
        print(cellData["date"] as! String)
        print(cellData["monthValue"] as! String)
        print(cellData["year"] as! String)
...
//        if(event.now_showing){
//            cell.noOfDays.text = event.eventdate.getCountDown(type: "number")
//            cell.noOfDaysLabel.text = event.eventdate.getCountDown(type: "label")
//            cell.countdownLabel.text = "Time In"
//        } else {
//            cell.noOfDays.text = event.eventdate.getDaysDifference(type: "number")
//            cell.noOfDaysLabel.text = event.eventdate.getDaysDifference(type: "label")
...
//        let nextViewController = storyBoard.instantiateViewController(withIdentifier: "EventDetailsViewController") as! EventDetailsViewController
//        nextViewController.event = event
//        nextViewController.navigationController?.title = "\(event.player1fname) vs \(event.player2fname)"
...
extension SchedulesViewController: HandleFilter {
